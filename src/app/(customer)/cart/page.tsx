...
import { FaTrashAlt, FaCalendarAlt, FaClock, FaUser, FaTag, FaTimes } from "react-icons/fa";
...
export default function CartPage() {
  const { cart, removeFromCart, clearCart, appliedPromoCode, applyPromoCode, removePromoCode } = useCart();
                  ...
                  <form onSubmit={handleApplyPromoCode} className="mb-4">
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 px-4 py-2 border border-[#BFA181] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#BFA181] text-[#7C5E3C]"
                        disabled={isApplyingCode}
                      />
                      <button
                        type="submit"
                        disabled={isApplyingCode || !promoCodeInput.trim()}
                        className="px-4 py-2 bg-[#BFA181] text-white rounded-lg font-semibold hover:bg-[#A68A64] transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                      >
                        <FaTag />
                ...
                <div className="flex flex-col gap-1 w-full">
                  ...
                  <div className="flex flex-row justify-between items-center mt-1 border-t pt-2">
                    ...
                    <span className="font-bold text-2xl text-[#BFA181]">
                      {totalAfterDiscount.toFixed(2)} NOK
                    </span>
                  </div>
                </div>
              </div>
